...
namespace {
struct CameraNameMapping {
    const char* key;
    const char* parameter_prefix;
};

constexpr std::array<CameraNameMapping, 4> kCameraMappings = {{
    {"frontCamera", "front_camera"},
    {"leftCamera", "left_camera"},
    {"rightCamera", "right_camera"},
...
public:
    SurroundVisionNode() : Node("surround_vision_node") {
        ...
        if (model_path_.empty()) {
            model_path_ = package_share_directory_ + "/models/smartCar/smartCar.obj";
            RCLCPP_WARN(this->get_logger(), "Model path not provided, using default: %s", model_path_.c_str());
        }
        
        if (camera_config_path_.empty()) {
            camera_config_path_ = package_share_directory_ + "/config/camerasParam.yaml";
        ...
        loadCameraTopicParameters();
        logCameraInputConfiguration();
...
private:
    ...
    std::unordered_map<std::string, CameraTopicConfig> camera_topic_configs_;
    CameraTopicConfig inside_camera_topic_config_;
    CameraTopicConfig ground_camera_topic_config_;
    ...
    static std::string transportToString(bool use_compressed) {
        return use_compressed ? "compressed" : "raw";
    }

    bool parseTransportMode(const std::string& value, bool default_value) {
        std::string lowered = value;
        std::transform(lowered.begin(), lowered.end(), lowered.begin(),
                       [](unsigned char c) { return static_cast<char>(std::tolower(c)); });

        if (lowered == "compressed" || lowered == "jpeg" || lowered == "jpg") {
            return true;
        }
        if (lowered == "raw" || lowered == "image" || lowered == "raw_image") {
            return false;
        }

        RCLCPP_WARN(this->get_logger(),
                    "Unknown transport mode '%s'. Expected 'raw' or 'compressed'. Using default '%s'.",
                    value.c_str(), transportToString(default_value).c_str());
        return default_value;
    }

    CameraTopicConfig readCameraTopicConfig(const std::string& parameter_prefix,
                                            const std::string& default_compressed_topic,
                                            const std::string& default_raw_topic) {
        const bool default_use_compressed = use_compressed_images_;
        const std::string topic_param = parameter_prefix + ".topic";
        const std::string transport_param = parameter_prefix + ".transport";

        const std::string topic_default = default_use_compressed ? default_compressed_topic : default_raw_topic;
        auto topic = this->declare_parameter<std::string>(topic_param, topic_default);

        const std::string transport_default = transportToString(default_use_compressed);
        auto transport = this->declare_parameter<std::string>(transport_param, transport_default);
        bool use_compressed = parseTransportMode(transport, default_use_compressed);

        if (topic.empty()) {
            RCLCPP_WARN(this->get_logger(),
                        "Camera input '%s' is disabled because no topic was provided.",
                        parameter_prefix.c_str());
        }
        return {topic, use_compressed};
    }

    void loadCameraTopicParameters() {
        camera_topic_configs_.clear();

        auto compressed_topic_for = [](const std::string& name) -> std::string {
            if (name == "frontCamera") return "/frontCamera/v4l2/compressed";
            if (name == "leftCamera") return "/leftCamera/v4l2/compressed";
            if (name == "rightCamera") return "/rightCamera/v4l2/compressed";
            if (name == "rearCamera") return "/rearCamera/v4l2/compressed";
            return "";
        };

        auto raw_topic_for = [](const std::string& name) -> std::string {
            if (name == "frontCamera") return "/frontCamera/image";
            if (name == "leftCamera") return "/leftCamera/image";
            if (name == "rightCamera") return "/rightCamera/image";
            if (name == "rearCamera") return "/rearCamera/image";
            return "";
        };

        for (const auto& mapping : kCameraMappings) {
            camera_topic_configs_[mapping.key] = readCameraTopicConfig(
                mapping.parameter_prefix,
                compressed_topic_for(mapping.key),
                raw_topic_for(mapping.key));
        }

        inside_camera_topic_config_ = readCameraTopicConfig(
            "inside_camera",
            "/insideCamera/v4l2/compressed",
            "/insideCamera/image");

        ground_camera_topic_config_ = readCameraTopicConfig(
            "ground_camera",
            "/groundCamera/v4l2/compressed",
            "/groundCamera/image");
    }

    void logCameraInputConfiguration() const {
        for (const auto& mapping : kCameraMappings) {
            const auto it = camera_topic_configs_.find(mapping.key);
            if (it == camera_topic_configs_.end()) {
                continue;
            }
            ...
            if (cfg.enabled()) {
                RCLCPP_INFO(this->get_logger(),
                            ...
                            mapping.key, cfg.topic.c_str(), transportToString(cfg.use_compressed).c_str());
            } else {
                RCLCPP_INFO(this->get_logger(),
                            "[%s] subscription disabled", mapping.key);
            }
        }

        if (inside_camera_topic_config_.enabled()) {
            RCLCPP_INFO(this->get_logger(),
                        "[insideCamera] subscribing to '%s' (%s)",
                        inside_camera_topic_config_.topic.c_str(),
                        transportToString(inside_camera_topic_config_.use_compressed).c_str());
        } else {
            RCLCPP_INFO(this->get_logger(), "[insideCamera] subscription disabled");
        }

        if (ground_camera_topic_config_.enabled()) {
            RCLCPP_INFO(this->get_logger(),
                        "[groundCamera] subscribing to '%s' (%s)",
                        ground_camera_topic_config_.topic.c_str(),
                        transportToString(ground_camera_topic_config_.use_compressed).c_str());
        } else {
            RCLCPP_INFO(this->get_logger(), "[groundCamera] subscription disabled");
        }
    }
    ...
    void initializeSubscribers() {
        ...
        for (const auto& mapping : kCameraMappings) {
            auto it = camera_topic_configs_.find(mapping.key);
            if (it == camera_topic_configs_.end()) {
                continue;
            }

            const CameraTopicConfig& cfg = it->second;
            if (!cfg.enabled()) {
                continue;
            }

            const std::string camera_name(mapping.key);
            if (cfg.use_compressed) {
                subscribers_.push_back(this->create_subscription<sensor_msgs::msg::CompressedImage>(
                    cfg.topic, qos,
                    [this, camera_name](const sensor_msgs::msg::CompressedImage::SharedPtr msg) {
                        processCompressedImage(camera_name, msg);
                    }));
            } else {
                subscribers_.push_back(this->create_subscription<sensor_msgs::msg::Image>(
                    cfg.topic, qos,
                    [this, camera_name](const sensor_msgs::msg::Image::SharedPtr msg) {
                        processRawImage(camera_name, msg);
                    }));
            }
        }

        if (inside_camera_topic_config_.enabled()) {
            if (inside_camera_topic_config_.use_compressed) {
                subscribers_.push_back(this->create_subscription<sensor_msgs::msg::CompressedImage>(
                    inside_camera_topic_config_.topic, qos,
        ...
        if (ground_camera_topic_config_.enabled()) {
            if (ground_camera_topic_config_.use_compressed) {
                subscribers_.push_back(this->create_subscription<sensor_msgs::msg::CompressedImage>(
                    ground_camera_topic_config_.topic, qos,
                    [this](const sensor_msgs::msg::CompressedImage::SharedPtr msg) {
    ...
    void processCompressedImage(const std::string& camera_name, const sensor_msgs::msg::CompressedImage::SharedPtr msg) {
        try {
            if (!isJpegCompressedMessage(msg, camera_name)) {
                return;
            }
    ...
    void processInsideCameraCompressed(const sensor_msgs::msg::CompressedImage::SharedPtr msg) {
        try {
            if (!isJpegCompressedMessage(msg, "inside_camera")) {
                return;
            }
    ...
    void processGroundCameraCompressed(const sensor_msgs::msg::CompressedImage::SharedPtr msg) {
        try {
            if (!isJpegCompressedMessage(msg, "ground_camera")) {
                return;
            }
    ...
    void calculateBowlUVMapping() {
        for (auto& vertex : bowl_vertices_) {
            ...
            for (size_t cam_id = 0; cam_id < kCameraMappings.size(); ++cam_id) {
                const std::string cam_key = kCameraMappings[cam_id].key;
                auto& cam = cameras_[cam_key];
    ...
    void initializeCameraTextures() {
        ...
        for (size_t i = 0; i < kCameraMappings.size(); ++i) {
            const std::string cam_name = kCameraMappings[i].key;
            ...
            auto cam_cfg_it = cameras_.find(cam_name);
            if (cam_cfg_it == cameras_.end()) {
                RCLCPP_WARN(this->get_logger(), "Skipping texture initialization for %s; camera intrinsics not found.", cam_name.c_str());
                continue;
            }
            auto& cam_cfg = cam_cfg_it->second;

            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, cam_cfg.width, cam_cfg.height, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
            texture_initialized_[i] = false;
            
            // Pre-allocate undistorted image buffer for this camera
            const auto& topic_cfg = camera_topic_configs_[cam_name];
            if (topic_cfg.use_compressed) {
                distorted_images_[cam_name] = cv::Mat::zeros(cam_cfg.height, cam_cfg.width, CV_8UC3);
            } else {
                distorted_images_.erase(cam_name);
            }
    ...
    bool isJpegCompressedMessage(const sensor_msgs::msg::CompressedImage::SharedPtr& msg,
                                 const std::string& stream_label) const {
        if (!msg) {
            RCLCPP_WARN(this->get_logger(),
                        "%s: compressed message is null; skipping decode",
                        stream_label.c_str());
            return false;
        }
        ...
        if (format.empty()) {
            RCLCPP_WARN(this->get_logger(),
                        "%s: compressed message has empty format; skipping decode",
                        stream_label.c_str());
            return false;
        }
        ...
        if (format.find("jpeg") == std::string::npos && format.find("jpg") == std::string::npos) {
            RCLCPP_WARN(this->get_logger(),
                        "%s: compressed message format '%s' is not JPEG; skipping decode",
                        stream_label.c_str(),
                        msg->format.c_str());
            return false;
        }
    ...
    void renderCameraFrustums() {
        ...
        for (size_t i = 0; i < kCameraMappings.size(); ++i) {
            const std::string cam_name = kCameraMappings[i].key;
            auto cam_it = cameras_.find(cam_name);
